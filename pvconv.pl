...
my(@opt_table) =   (
		    ...
		    "outtype=s",     # output type: nifti, nifti_img, analyze or minc
...
# and write output image
	...
	write_analyze_image(\%ghdr, \%bhdr, $rawfile, $outfile, 0);
    } 
    elsif ($options{outtype} eq 'nifti_img') {
	write_analyze_image(\%ghdr, \%bhdr, $rawfile, $outfile, 1);
    } 
    elsif ($options{outtype} eq 'nifti') {
	write_analyze_image(\%ghdr, \%bhdr, $rawfile, $outfile, 2);
    } 
...
sub write_analyze_image {
    my($ghdr, $bhdr, $rawfile, $outfile, $isnifti) = @_;

# create a template analyze header
    my $h;
    if ($isnifti) { 
    	$h = Data::Struct::Nifti->new(); 
   	if ($isnifti==1) { 
        	$h->magic("ni1\0"); 
        	$h->vox_offset(0); 
        	$h->extension( "hdr" ); 
	} else {	# Analyze can't parse nift_ext in .hdr files, so
			# only use in .nii files
        	$h->vox_offset(352); 
        	push @{$h->structure_array}, [ "nifti_ext", 'l', 0 ]; 
	}	
    } else { 
	$h = Data::Struct::Analyze->new();
    }

# fill the analyze header
    if ((!$isnifti) && (defined($bhdr->{SUBJECT_name_string}))) {
	$h->patient_id($bhdr->{SUBJECT_name_string}) };
    if (defined($bhdr->{ACQ_protocol_name})) {
	$h->descrip($bhdr->{ACQ_protocol_name}) };
    if ((!$isnifti) && (defined($ghdr->{time_of_scan}))) {
	$h->exp_time($ghdr->{time_of_scan}) };
    if ((!$isnifti) && (defined($ghdr->{date_of_scan}))) {
...
# Do the header transposition stuff for each layer
	...
	if (defined($ghdr->{mat}[$ln])) { 
    		if ($isnifti) { 
			foreach $i (0..3) { 
				$hdr_layer[$ln]->srow_x( [$i], [ $ghdr->{mat}[$ln][0][$i] ] );
				$hdr_layer[$ln]->srow_y( [$i], [ $ghdr->{mat}[$ln][1][$i] ] );
				$hdr_layer[$ln]->srow_z( [$i], [ $ghdr->{mat}[$ln][2][$i] ] );
			}
			$hdr_layer[$ln]->sform_code( 1 );	
		} else { 
	...
	if ($isnifti) { 
		# Set the units
		my $xyzt_units = 2; # NIFTI_UNITS_MM
		if ($ghdr{dim}[3]>1) { 
			$xyzt_units = $xyzt_units | 16; # NIFTI_UNITS_MSEC
		}
		$hdr_layer[$ln]->xyzt_units( $xyzt_units );
		
		# Work out which dimensions are frequency, phase & slice
		my @dimtypes = ( 1, 2, 3 ); 
		my $is2d = ($bhdr->{ACQ_dim} == 2);
		my $code = $ghdr{transpose}[$ln]{code};
		if ( !$is2d ) { $dimtypes[2] = 0; }
		if (($code==1) || ($is2d && ($code==2))) { 
			$dimtypes[0] = 2; $dimtypes[1] = 1;
		} elsif ($code==2) { 
			$dimtypes[1] = $dimtypes[2]; $dimtypes[2] = 2;
		} elsif ($code==3) { 
			$dimtypes[0] = $dimtypes[2]; $dimtypes[2] = 1;
		}
		$hdr_layer[$ln]->dim_info( ($dimtypes[0]&0x03) | (($dimtypes[1]&0x03)<<2) | (($dimtypes[2]&0x03)<<4) ); 

		if ($is2d && ($ghdr{dim}[2]>1)) { 
			# Get the object order for this layer
			my @ln_order;
			my $max_slice=0; 
			my $min_slice=$ghdr{dim}[2]+1; 
			foreach $i (0..$ghdr{dim}[2]-1) { 
				$ln_order[$i] = ($ghdr{objorder}[$ln+($i*$ghdr->{layer_nr})]-$ln)/$ghdr->{layer_nr};
				if ($max_slice < $ln_order[$i]) { $max_slice = $ln_order[$i]; }
				if ($min_slice > $ln_order[$i]) { $min_slice = $ln_order[$i]; }
			}
			# Examine slice ordering for each layer
			my $diff = $ln_order[1] - $ln_order[0]; 	
	   		my $fits_obj = 0;
			if ($diff==1) {
				$fits_obj = 1; 	# SLICE_SEQ_INC
				foreach $i (0..$ghdr{dim}[2]-1) { 
					if ($ln_order[$i] != $min_slice + $i) { $fits_obj = 0; } 
				}
			} elsif ($diff==2) { 
				$fits_obj = 3; 	# SLICE_ALT_INC
				my $half_pts = ($ghdr{dim}[2]+1)/2;
				foreach $i (0..$half_pts-1) { 
					if ($ln_order[$i] != $min_slice + (2*$i)) { $fits_obj = 0; } 
				}
				foreach $i (0..($ghdr{dim}[2]/2)-1) { 
					if ($ln_order[$half_pts+$i] != $min_slice+1 + (2*$i)) { $fits_obj = 0; } 
				}
			} elsif ($diff==-1) { 
				$fits_obj = 2; 	# SLICE_SEQ_DEC
				foreach $i (0..$ghdr{dim}[2]-1) { 
					if ($ln_order[$i] != $max_slice - $i) { $fits_obj = 0; } 
				}
			} elsif ($diff==-2) { 
				$fits_obj = 4; 	# SLICE_ALT_DEC
				my $half_pts = ($ghdr{dim}[2]+1)/2;
				foreach $i (0..$half_pts-1) { 
					if ($ln_order[$i] != $max_slice - (2*$i)) { $fits_obj = 0; } 
				}
				foreach $i (0..($ghdr{dim}[2]/2)-1) { 
					if ($ln_order[$half_pts+$i] != $max_slice-1 - (2*$i)) { $fits_obj = 0;  } 
				}
			} else { $fits_obj = 0; }

			if ($fits_obj) { 	# all parameters seem valid
				$hdr_layer[$ln]->slice_code( $fits_obj );
				$hdr_layer[$ln]->slice_start( $min_slice );
				$hdr_layer[$ln]->slice_end( $max_slice );
			
				if ($ghdr{dim}[3]>1) { 
					my $slice_time = $ghdr->{transpose}[$ln]{vox}[3] / 
						($ghdr->{layer_nr} * $ghdr{dim}[2] );
					$hdr_layer[$ln]->slice_duration( $slice_time ); 	
						# Should consider PackDel?
				}
			} elsif (!$options{quiet}) {
				warn ("Can't derive slice ordering from header. Slice ordering not stored " .
					"in Nifti file header");
			}
		}
	}
...
# make hdr and mat info, open img/nii files
    $k = 0;
	...
	foreach $ln (0..$ghdr->{layer_nr}-1) {
		...
		if ((!$isnifti) && defined($ghdr->{mat}[$ln]));
	    
	    open($outhandles[$k], 
	    	($isnifti==2) ? ">>$outnames[$k].nii" : "+>$outnames[$k].img") or 
		die ("Error opening file: $outnames[$k]." . ($isnifti==2) ? "nii" : "img");
	...
	if (!$options{quiet}) {
	    if ($isnifti==2) { 
	   	print "Written nifti image $outnames[$o].nii\n";
	    } elsif ($isnifti==1) { 
		print "Written nifti image $outnames[$o].img\n";
	    } else { 
...
Options:
    ...
    -outtype       output type: "nifti", "nifti_img", "analyze" or "minc"
...
Specify output type, currently one of "nifti", "nifti_img", "analyze" or "minc".  
"analyze" is the default.
